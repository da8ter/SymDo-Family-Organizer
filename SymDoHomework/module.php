 ...
 * (tools/uebernahme-webapp.py), samt locale.json. Wer an der Liste etwas
 * aendern will, aendert sie in SymDoWebApp/module.html und uebernimmt neu — von
 * Hand wird hier nichts nachgepflegt, sonst laufen die beiden Oberflaechen
 * auseinander. Dasselbe Verfahren wie bei SymDoNotes und SymDoEdumaps.
 *
 * Der Unterschied zu den anderen Kacheln ist EINE Zeile im Zustand: `tabs`
 * nennt nur `homework`. Bei einem einzigen Bereich laesst die Web-App die
 * Leiste ohnehin weg (html.no-tabs).
 *
 * Anders als bei den Klassenseiten wird hier GESCHRIEBEN: eine Hausaufgabe
 * anlegen, abhaken, aendern, loeschen. Was aus WebUntis kommt, bleibt auch in
 * dieser Kachel unantastbar — der Riegel sitzt im Gateway, nicht hier.
 *
 * Die Aufgaben liegen im Gateway und reisen ueber dessen AiCall-Relay:
 * aiPost('/homework', …) → RequestAction('AiCall') →
 * IPS_RequestAction($gateway, 'AiTileRequest', …) → HomeworkHandleAction().
 * Einen Token hat die Kachel nicht, dieser Weg ist der einzige — und dafuer
 * MUSS die GUID dieses Moduls in der Weissliste des Gateways stehen
 * (AppCore::IsSymDoWebAppInstance), sonst wartet die Kachel auf eine Antwort,
 * die nie kommt. Denselben Weg nimmt der Stundenplan: aus seinem Fachkatalog
 * kommen Symbol und Farbe des Fachs, und aus dem Plan des Kindes die Fachliste
 * des Blattes.
 */
class SymDoHomework extends IPSModuleStrict
{
    ...
    public function RequestAction(string $Ident, mixed $Value): void
    {
        switch ($Ident) {
            ...
            case 'AiCall':
                ...
                return;
            case 'AiResult':
                $this->AufgabenAntwort((string)$Value);
                return;
     ...
     *
     * `gatewayAvailable` ist PFLICHT: visibleTabs() blendet den
     * Hausaufgaben-Bereich ohne Gateway aus (homeworkMoeglich), und ohne
     * sichtbaren Bereich zeigte die Kachel den Rueckfall (die leere
     * Uebersicht).
     */
    private function Zustand(): string
    {
        ...
        return (string)json_encode(KachelStand::MitPruefwert([
            ...
            /* Die Mitglieder MUESSEN mit: aus ihnen kommen die Kinder (persona
               ...
               stehen in der Filterleiste und an den Zeilen. */
            'users'            => $this->Mitglieder(),
            /* Kein Vorgabe-Mitglied: eine Hausaufgabe gehoert einem KIND, und
               welches es ist, steht im Blatt zur Wahl (oder ist durch die
               Filterleiste schon entschieden). */
            'defaultUserID'    => '',
            /* PFLICHT: homeworkMoeglich() haengt daran. Ohne das Feld waere der
               Bereich unsichtbar, und die Kachel fiele auf ein leeres Dashboard
               zurueck. */
            'gatewayAvailable' => $gw > 0,
            /* Kein Zauberstab in dieser Kachel: die Eingabezeile tippt eine
               Notiz, sie erkennt nichts. */
            'aiEnabled'        => false,
            'tabs'             => [
                'dashboard' => false,
                'shopping'  => false,
     ...
     * Pfadunabhaengig: die Kachel schickt durch, was der Client nennt
     * (/homework, /timetable), und AiRelayBody entscheidet.
     */
    private function AufgabenRelay(string $json): void
    {
    ...
    private function AufgabenAntwort(string $json): void
    {
     ...
     * dieses Modul tut — Hausaufgaben, Stundenplan, Mitglieder —, liegt in der App-Haelfte des
     ...
     * haben — sie zeigte dann andere Mitglieder und andere Aufgaben als die App.
    ...
    public function GetConfigurationForm(): string
    {
        ...
        $stand = $gw > 0
            ...
            : $this->Translate('No SymDo Gateway found — the homework lives there. Create one first.');

        return (string)json_encode([
            'elements' => [
                ['type' => 'Label', 'caption' => $this->Translate('Shows the homework of the children as a tile — the same list, the same view as in the app. There is nothing to set up here: the homework lives in the SymDo Gateway.')],
                ['type' => 'Label', 'caption' => $stand],
                ['type' => 'Label', 'caption' => $this->Translate('Homework from WebUntis is shown, not edited: what the school ticked off stays ticked off. Everything entered here can be changed and deleted here.')],
            ],
            'actions' => [
