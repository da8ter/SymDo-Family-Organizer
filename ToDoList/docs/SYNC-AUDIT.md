...
**Release-Check (2026-07-07):** Migrations-Lücke gefunden und behoben — der Merge-Skip-Pfad lieferte Bestands-Tasks (vor v2.7 synchronisiert) das `caldavRaw` nicht nach; die erste Bearbeitung nach dem Update hätte fremde VALARM/RRULE zerstört, und die veralteten lokalen Defaults (recurrence `none`, `dueAllDay` false) hätten als „lokale Änderung" die Server-RRULE/`VALUE=DATE` gelöscht. Fix: Backfill direkt nach dem Server-Match (Raw + parse-abgeleitete Felder, nur bei lokalen Defaults; lokal gesetzte Werte wie Reminder/eigene Recurrence bleiben unangetastet). 12 Unit-Tests + CalDAV-Live-Regression 23/23. Locale-Vollständigkeit (PHP+JS) geprüft: 0 fehlend.

**Reminder-Flip-Flop behoben (2026-07-07, „Option B — Aus ist autoritativ"):** Das explizite Ausschalten der Erinnerung (`notification: true → false`) entfernt jetzt neben dem eigenen markierten Alarm auch **fremde relative DISPLAY-Alarme** — sie sind genau die Quelle des importierten Reminder-Zustands und hätten sonst (a) in anderen Clients weitergeklingelt und (b) sich beim nächsten Server-Edit als „an" reimportiert (Flip-Flop). EMAIL-/AUDIO- und absolute Alarme bleiben unantastbar; unbeteiligte Edits erhalten weiterhin alles byte-genau. Begleit-Fix: `UpdateItem` clampt die Vorlaufzeit nur noch, wenn der Payload Erinnerungs- oder Fälligkeits-Felder enthält (vorher clampte jede Titeländerung still den Lead — bestätigter Review-Befund). Verifiziert: 14 Unit-Tests (alle Übergänge: aus/Lead-Änderung/an/Titel-only, EMAIL/absolute unangetastet) + live gegen ownCloud (12h-Alarm überlebt Titel-Edit trotz naher Fälligkeit; Aus entfernt DISPLAY-Alarm, EMAIL bleibt; Server-Edit reaktiviert die Erinnerung nicht mehr) + CalDAV-Regression 23/23.
